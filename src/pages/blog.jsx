      ...
      <main className="pt-36 mx-6 md:mx-28">
        {blogPosts?.items?.map((post) => (
          <section className="mb-16" key={post.sys.id}>
            {"< Back"}
            <h1 className="mt-4 text-2xl md:text-4xl font-bold mb-6">{post.fields.title}</h1>
            ...
            <cite className='text-gray-500 underline text-sm'>{post.sys.createdAt}</cite>
            <div className=" mt-5">
              <p>{post.fields.description}</p>
            </div>
            <div className="main-with-sidebar parent">
              <div className="children mb-6 pr-12"></div>
